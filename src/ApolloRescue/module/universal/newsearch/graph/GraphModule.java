...
public class GraphModule extends AbstractModule{
    ...
    public HashMap<EntityID, AreaGraph> hareas = new HashMap<EntityID, AreaGraph>();
    public double worldGridSize = 22085.68D;
    public LinkedList<AreaGraph> gasStations = new LinkedList<>();


//    public GraphModule(ApolloWorld world) {
//        this.world = world;
//        createGraph();
//    }
    ...
    public GraphModule(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        ...
        Set<StandardEntity> roads = new HashSet<>(getRoadsWithURN());
        Set<StandardEntity> buildings = new HashSet<>(getBuildingsWithURN());
...
//                    || reversePair.first().equals(myEdge.getNodes().first().getId()) && reversePair.second().equals(myEdge.getNodes().second().getId())) {
    ...
    public List<Node> getAreaNodes(EntityID areaId) {   //A_Star
        /**
    ...
    private void createGraph() {
        /**
         * method asli baraye sakhtane graphModule.
        ...
        for (StandardEntity ent : wi.getAllEntities()) {
            if (ent instanceof Area) {
                Area area = (Area) ent;
                AreaGraph ag = new AreaGraph(area, this);
                hareas.put(ent.getID(), ag);
                if (ag.areaType == AreaGraph.AREA_TYPE_GAS_STATION) {
                    gasStations.add(ag);
                }
            }
        }
...
//        int size = (int) (MRLConstants.AGENT_SIZE * 1.5d);
        ...
        for (Edge edge : area.getEdges()) {
            if (edge.isPassable() /*&& EdgeComponent.getEdgeLength(edge) >= size*/) { //todo: Mostafa merge edges
    ...
    private void createAreNodes(Area area, ArrayList<Pair<Integer, Integer>> pairs) {
        ...
        boolean isEntrance = getEntrances().contains(area);  //TODO
...
//                        myEdges.put(edgeId, myEdge);
        ...
        // only for draw graphModule on viewer and does not another cost.  TODO
