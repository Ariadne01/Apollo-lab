...
import ApolloRescue.module.complex.component.PropertyComponent;
...
public class FireZoneBuildingEstimator {
    public FireZoneBuildingEstimator(ApolloWorld world) {
        this.world = world;
        this.maxExtinguishDistance = world.getMaxExtinguishDistance();
        propertyComponent = world.getComponent(PropertyComponent.class);
    }

    private ApolloWorld world;
    private PropertyComponent propertyComponent;

    private static final double INITIAL_COST = 200;   //mrl is 500

    private static final double AGENT_SPEED = 40000;   // mrl is 32000
    private static final double BASE_PER_MOVE_COST = 10;  //mrl is 30
...
//    private static final double CONFIRM_ALONE_DISTANCE = 100000;
    ...
    private BuildingModel lastTarget;

    public void updateFor(Cluster targetCluster, BuildingModel lastTarget) {
    ...
    public int getCost(BuildingModel fireBuilding) {
//    	System.out.println("Time: " + world.getTime() + " Agent Id: " + world.getSelfHuman().getID() + "\t调用了FireZoneBuidingEstimator.getCost()方法");

        double cost = INITIAL_COST;

        FireBrigade me = (FireBrigade) world.getSelfHuman();
        Building building = fireBuilding.getSelfBuilding();
//        System.out.println("Time: " + world.getTime() + " Agent Id: " + world.getSelfHuman().getID() + "\tBuiding Id : " + building.getID());
        ...
        // fuel
        boolean isRecentlyUpdated = (world.getTime() - propertyComponent.getEntityLastUpdateTime(building) <= RECENT_UPDATE_TIME_MAX);
        if (currentEstimatedFieryness == StandardEntityConstants.Fieryness.HEATING) {
            double award;
...
//        // need confirm?
    ...
    public boolean shouldConfirmBuildingCondition(BuildingModel fireBuilding) {
        Building b = fireBuilding.getSelfBuilding();

        int timeNow = world.getTime();
        int buildingUpdateTime = propertyComponent.getEntityLastUpdateTime(b);
        ...
        double needExtinguishWater = ExtinguishHelper.waterNeededToExtinguish(fireBuilding) * 0.7;
