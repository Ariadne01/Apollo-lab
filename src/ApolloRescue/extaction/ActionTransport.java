...
public class ActionTransport extends ExtAction {
    ...
    public ActionTransport(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo, ModuleManager moduleManager, DevelopData developData) {
        ...
        switch  (scenarioInfo.getMode()) {
            case PRECOMPUTATION_PHASE:
                this.pathPlanning = moduleManager.getModule("ActionTransport.PathPlanning", "adf.component.module.algorithm.PathPlanning");
                break;
            case PRECOMPUTED:
                this.pathPlanning = moduleManager.getModule("ActionTransport.PathPlanning", "adf.component.module.algorithm.PathPlanning");
                break;
    ...
    public ExtAction calc() {
        ...
        if(this.target != null) {
            ...
            if(this.result==null) {
                System.out.println("没有Action" + this.agentInfo.getID()+ " "+this.agentInfo.getTime());
            }
            else{
                System.out.println("有Action" + this.agentInfo.getID()+ " "+this.agentInfo.getTime());
            }
        }
        else{
            System.out.println("没有目标" + this.agentInfo.getID()+ " "+this.agentInfo.getTime());
        }
        return this;
    }
        ...
        if(targetEntity instanceof Human) {
            ...
            if (agentPosition.getValue() == targetPosition.getValue()) {
                if (human.isBuriednessDefined() && human.getBuriedness() > 0) {
                   // System.out.println("此时在救人" + this.agentInfo.getID() + " "+ this.agentInfo.getTime());
                    return new ActionRescue(human);
                } else if (human.getStandardURN() == CIVILIAN) {
                   // System.out.println("此时在load" + this.agentInfo.getID() + " "+ this.agentInfo.getTime());
                    return new ActionLoad(human.getID());

                }
            } else {
               // System.out.println("不在同一地点" + this.agentInfo.getID() + " "+ this.agentInfo.getTime());
                List<EntityID> path = pathPlanning.getResult(agentPosition, targetPosition);

                if (path != null && path.size() > 0) {
               //     System.out.println("move　到　human" + this.agentInfo.getID() + " "+ this.agentInfo.getTime());
