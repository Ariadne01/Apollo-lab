...
public class ChangeSet {
     ...
     */
    public ChangeSet() {
        changes =new ConcurrentHashMap<EntityID, Map<String, Property>>() {
            @Override
        ...
        /*new LazyMap<EntityID, Map<String, Property>>() {
            @Override
        ...
        deleted = new CopyOnWriteArraySet<>();
        entityURNs = new ConcurrentHashMap<>();
    }
    ...
    */
    public Set<Property> getChangedProperties(EntityID e) {
        return new HashSet<>(changes.get(e).values());
    }
    ...
    */
    ...
    public static ChangeSet filter(ChangeSet changeSet, ChangeSet filter){
        ...
        for(Map.Entry<EntityID, Map<String, Property>> next : changeSet.changes.entrySet()){
            ...
            if(!isFiltered){
                next.getValue().values().stream().filter(p -> e != null && urn != null && p != null).forEach(p -> {
    ...
    public void mergeWithFilter(ChangeSet changeSet, ChangeSet filter){
        ...
        for(Map.Entry<EntityID, Map<String, Property>> next : changeSet.changes.entrySet()){
            ...
            found = filter != null ? filter.changes.get(0) : null;
            if(found == null){
                next.getValue().values().stream().filter(p -> e != null && urn != null && p != null).forEach(p -> {
    ...
    */
    public void addAll(Collection<? extends Entity> c) {
        for (Entity entity : c) {
            entity.getProperties().stream().filter(Property::isDefined).forEach(property -> {
    ...
    */
    public void debug() {
        ...
        for (Map.Entry<EntityID, Map<String, Property>> next : changes.entrySet()) {
            ...
            for (Property property : next.getValue().values()) {
                Logger.debug("    " + property);
            }
        }
        for (EntityID aDeleted : deleted) {
            Logger.debug("  Deleted: " + aDeleted);
        }
    }
}
